    ...
    <main className="mx-auto min-h-screen max-w-6xl px-5 py-6 sm:px-8">
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-[200px_1fr]">
        <FilterRail
          ...
          total={total}
        />
        <MessageStream messages={visible} status={status} totalCount={total} />
      </div>
...
function SignalMeter({
  ...
  return (
    <section className="mt-6 rounded-lg border border-line bg-panel p-5">
      ...
      <div
        className="relative h-7 w-full overflow-hidden rounded-md bg-panel2"
        role="img"
        aria-label={
          total === 0
            ? "Awaiting messages"
            : `Sentiment breakdown: ${order
                .filter((s) => counts[s] > 0)
                .map((s) => `${counts[s]} ${SENTIMENT_META[s].label}`)
                .join(", ")}`
        }
      >
        {total === 0 ? (
...
function FilterRail({
  ...
  return (
    <nav
      ...
      className="rounded-lg border border-line bg-panel p-2 lg:h-fit"
    >
    ...
    if (totalCount > 0) {
      return (
        <Panel>
          <EmptyState
            title="No matches"
            body="No messages match this filter. Try switching to All or selecting a different sentiment."
          />
        </Panel>
      );
    }
  ...
  return (
    <Panel>
      ...
      <p className="border-t border-line px-4 py-2 font-mono text-[11px] text-muted">
        showing {messages.length} of {totalCount} message
        {totalCount === 1 ? "" : "s"}
      </p>
    </Panel>
  );
  ...
  return (
    <div className="flex items-start gap-4 px-4 py-3">
      <span
        ...
        style={{ color: meta.dot, backgroundColor: hexToRgba(meta.dot, 0.1) }}
      >
        <span
          aria-hidden="true"
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm text-text" title={m.snippet}>
          {m.snippet}
        </p>
      ...
      {/* Mobile: confidence as plain text */}
      <span className="shrink-0 self-center font-mono text-[11px] text-muted sm:hidden">
        {conf}%
      </span>

      {/* Desktop: confidence bar + text */}
